...
public class Clasificacion extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        clasificacion.setOnItemClickListener((parent, view, position, id) ->{
            String equipoSeleccionado = clasificacionEquipos.get(position);
            String [] partesEquipo = equipoSeleccionado.split("º - | - ");
            editTextPosicionEquipo.setText(partesEquipo[0]);
            editTextNombreEquipo.setText(partesEquipo[1]);
            editTextPuntosEquipo.setText(partesEquipo[2]);
        });
    ...
    public void añadirEquipoClasificacion(View view){
        ...
        if(!nombreEquipo.isEmpty() && !posicion.isEmpty() && !puntos.isEmpty()){
            String nuevoEquipo = (posicion + "º" + " - " + nombreEquipo + " - " + puntos);
    ...
    public void editarEquipoClasificacion(View view){
        ...
        if(!nombreEquipo.isEmpty() && !posicion.isEmpty() && !puntos.isEmpty()){
           int indice = clasificacion.getCheckedItemPosition();
           if(indice != ListView.INVALID_POSITION){
               String equipoEditado =(posicion + "º" + " - " + nombreEquipo + " - " + puntos);
